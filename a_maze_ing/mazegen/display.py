...
def render_maze_base(maze: Any) -> list[list[str]]:
    """Convert the maze into colored cells for the standard output"""
    t = theme()
    canvas = []
...
def overlay_path(canvas: list[list[str]], maze: Any,
    ...
    """Change the color of the cells that are part of the path."""
    t = theme()
    path_block = t["path"]
...
def print_canvas(canvas: list[list[str]]) -> None:
    """Print the canvas to standard output."""
    for row in canvas:
        print("".join(row))


def display_maze(maze: Any, solution: bool) -> None:
    """Clear the terminal and call the print_canvas function."""
    print("\033[3J\033[2J\033[H", end="")
    print("\033[2J\033[H", end="")

    if not maze.logo:
        print(
                "[ERROR] Maze too small for pattern ‘42’.\n"
                "The height and width must be greater than 9.\n"
                "Pattern omitted."
                )
